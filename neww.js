...
const clientSocket = require('socket.io-client')('http://127.0.0.1:3000', { transports: ['websocket'] });
...
const exit = (code = '0') => {
	...
	request('http://127.0.0.1:3000' + '/noUseAccount?' + account, () => {
		process.exit(code)
	})
}
...
clientSocket.on('canRun', async () => {
	...
	request('http://127.0.0.1:3000' + '/' + accountType, (error, response, body) => {
